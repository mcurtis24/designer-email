...
function HeadingBlock({ block, isSelected, onClick, onFormatRequest, onActiveStatesChange }: HeadingBlockProps) {
  ...
  const handleFormat = (command: string, value?: string) => {
    ...
    // Handle special cases that need custom update logic
    if (command === 'headingLevel' && value) {
      const level = parseInt(value) as 1 | 2 | 3

      // Save current content before changing level
      const currentContent = contentRef.current?.innerHTML || data.text

      // Update typography based on heading level (Phase 3: Canva redesign)
      const typographyByLevel = {
        1: { fontSize: '48px', fontWeight: 700, lineHeight: 1.2, color: '#111827', letterSpacing: '-0.02em', fontFamily: 'Georgia, serif' },
        2: { fontSize: '36px', fontWeight: 600, lineHeight: 1.3, color: '#1F2937', letterSpacing: '-0.01em', fontFamily: 'Georgia, serif' },
        3: { fontSize: '28px', fontWeight: 600, lineHeight: 1.4, color: '#374151', letterSpacing: '0', fontFamily: 'Georgia, serif' },
      }

      // Update block with new level and typography
      updateBlock(block.id, {
        data: {
          ...data,
          level,
          text: currentContent,
          ...typographyByLevel[level]
        }
      })

      // Force re-render and restore focus
    ...
    try {
      ...
      const newText = sanitizeEmailHTML(contentRef.current.innerHTML)
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (!isEditing) {
      ...
      // Enter editing mode
      setIsEditing(true)
      setEditingBlock(block.id, 'heading')
    }
  }
  ...
  return (
    <div className="relative">
      {isEditing ? (
        ...
        <div
          onClick={handleClick}
          className={`cursor-pointer transition-all border-2 border-transparent ${
          ...
          <HeadingTag
            ...
            dangerouslySetInnerHTML={{
              __html: DOMPurify.sanitize(data.text || '<span style="color: #9ca3af;">Click to add heading...</span>', {
                ALLOWED_TAGS: ['span', 'strong', 'em', 'u', 'a'],
